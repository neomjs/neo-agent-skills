#!/usr/bin/env node
/**
 * @summary Fails on credential-shaped literals: in the files it is given (a pre-commit hook passes the staged ones),
 ...
 * secret scanner catches it out of band, after the fact; this guard is the tripwire at the commit and the pull request.
 *
...
/**
 * `{kind, re}` per credential shape. Each `re` is global and bounded on both sides, so a longer run of the same
 * alphabet is not read as a key.
...
function isText(file) {
    ...
    const fd = openSync(file, 'r'), head = Buffer.alloc(8000);
...
export function run(files) {
    ...
    files.forEach(file => {
        if (!existsSync(file) || !statSync(file).isFile() || !isText(file)) {
            return
        }

        const findings = findSecrets(readFileSync(file, 'utf8'));
...
// Canonicalized on BOTH sides, as in check-substrate-size.mjs: a package `bin` is a symlink, and under
// `--preserve-symlinks-main` import.meta.url keeps the link path, so comparing either side unresolved lets the
// module load, skip `run()`, and exit 0 with nothing checked.
