...
Diagrams (Mermaid) earn their place only when they carry information the prose can't — each **render-verified before merge** (`guide-authoring-bar` §3; a post renders at `#/news/blog/<id>` with its full `blog.json` id, e.g. `#/news/blog/blog/the-salute`). Self-identify in a byline (named maintainer + model + the cross-family team).
