...
|---|---|---|---|
| **Package version** | `package.json` `version` | extend `buildScripts/release/prepare.mjs` to derive version-bearing structured surfaces + the Engine-owned [Neural Link guide](https://github.com/neomjs/neo/blob/dev/learn/agentos/NeuralLink.md) version line; coherence-check the rest | Engine-owned Neural Link guide; `FooterContainer.mjs` literal |
...
| **Identity handle** | `ai/graph/identityRoots.mjs` | route references through the seam. Account rename is operator-owned (Tier-4); the skill propagates. Model-version stays in the Brain-owned [Model Stats guide](https://github.com/neomjs/neo-agent-brain/blob/dev/learn/agentos/ModelStats.md) per ADR 0012 — do NOT de-version the prose | hand-maintained current surfaces can retain stale handles after a graph/code migration; re-run a seam-keyed grep |
| **Recurring motto** | Brain-owned [Dream Pipeline guide](https://github.com/neomjs/neo-agent-brain/blob/dev/learn/agentos/DreamPipeline.md) (origin of "the system evolves by predicting its own evolution") | reference a single quotable constant; don't re-type | Dream Pipeline (2×), README, ROADMAP |
| **Codebase-scale metrics** | Engine-owned [Codebase Overview](https://github.com/neomjs/neo/blob/dev/learn/guides/fundamentals/CodebaseOverview.md) | README "Platform at Scale" refreshes in lock-step; carry an explicit as-of date | README |
...
- Performance: Engine-owned [Speed guide](https://github.com/neomjs/neo/blob/dev/learn/benefits/body/Speed.md) — consistent **20,000+** DOM-updates/sec floor, observed peak **over 40,000**. Cite as observed-peak (+ floor), not a guaranteed benchmark.
