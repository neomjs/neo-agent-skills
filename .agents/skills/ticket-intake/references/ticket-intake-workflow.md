...
## §0 — Understand the intent, and challenge the fix, before you accept

Two questions before the validation sweep or branching. They fail differently, and **§0 sits outside the Hot Context Fast-Path** — both run even when §1 and §2 are skipped.

**Is the ticket still right?** Tickets go stale. Run `ticket-create` §0's intent read against the ticket you were handed rather than the one you would write — affected files, neighbors, imports, the subsystem's guide. Intake adds the question creation cannot ask: is the pattern it requests still architecturally current, or deprecated since? Slower on purpose — the judgment is the point.

**Is the prescribed fix right?** A ticket can be fresh, its problem real, and its solution wrong. §1 step 10 (**Hypothesis vs. Root Cause Validation**) already forbids accepting a prescribed solution blindly — but it sits *inside* the Validation Sweep, which the Hot Context Fast-Path and the self-authored carve both skip. §0 is where that question survives the skip; it is not a new one. Apply `ticket-create` §2 stage 2 (**Prescription**) against the code rather than the ticket's prose — name the mechanism the ticket prescribes, the class, module or layer you would subclass, extend or place beside; open it; then record the answer stage 2 actually asks for: `Prescription checked: <path> — owns the concern`, or `— better owner: <path>`. A layer that already owns the concern is the finding. This binds hardest on a ticket you authored — see [`./self-authored-carve.md`](./self-authored-carve.md), where same-session authorship removes stage 2's independence rather than supplying it.

**Three outcomes, not two.** Accept; reject (§4); or — the common case — **accept and sharpen.** Most tickets are not clear, and you now hold the hottest context anyone will have on this one. Route the sharpening by authorship, per `ticket-create` §11: on **your own** ticket, write what the exploration established into the body in one edit, so the next reader and the PR's reviewer inherit it instead of re-deriving it; on **someone else's**, propose it in a comment — their body and ACs stay theirs, with claimer-authored in-body sections (intake-derived ledgers) still yours to update. One edit or one comment, never a re-authoring.
...
6. **Duplication Check:** Semantic search is significantly more powerful than string matching.
   ...
   - **Fallback:** If you explicitly require exact keyword verification (e.g. a specific UUID or function name constraint), fall back to `query_documents` (archived included) or a live `gh search issues --owner neomjs "<token>"`.

7. **Contract Completeness Sweep (Readiness Gate):** If the ticket proposes modifying, introducing, or deprecating a surface that is consumed by humans, agents, or external systems (e.g., public APIs, configs, MCP tools), you MUST verify that a **Contract Ledger** matrix is present in the ticket body. See the [Contract Ledger protocol](https://github.com/neomjs/neo-agent-brain/blob/dev/learn/agentos/process/contract-ledger.md). This is a separate readiness gate that must pass before checking ticket reality.
   - If the matrix is missing or incomplete, the ticket enters the `needs-contract-alignment` state.
   - A matrix is incomplete when a row names an existing field, method, helper, tool, config key, docs path, or runtime surface that does not match current substrate reality or lacks the Surface-Anchor V-B-A required by the [Contract Ledger protocol](https://github.com/neomjs/neo-agent-brain/blob/dev/learn/agentos/process/contract-ledger.md).
