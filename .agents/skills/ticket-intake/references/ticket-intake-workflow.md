...
**Is the ticket still right?** Tickets go stale. Understand what the work is *for* from the affected files (intent belongs in their JSDoc — `src/core/Base.mjs` is the bar), their neighbors, their imports, and the subsystem's guide, plus `memory-mining` / `ask_knowledge_base`. That is also what confirms a requested pattern is still architecturally current rather than deprecated. Slower on purpose — the judgment is the point. Intent documented nowhere is the finding: ticket the gap, don't guess.

**Is the prescribed fix right?** A ticket can be fresh, its problem real, and its solution wrong: §1.3 validates the premise, and nothing else here validates the prescription. Apply `ticket-create` §2 stage 2 (**Prescription**) against the code rather than the ticket's prose — name the mechanism the ticket prescribes, the class, module or layer you would subclass, extend or place beside; open it; cite it as `Prescription checked: <path>`. A layer that already owns the concern is the finding. This binds hardest on a ticket you authored — see [`./self-authored-carve.md`](./self-authored-carve.md), where same-session authorship removes stage 2's independence rather than supplying it.

**Three outcomes, not two.** Accept; reject (§4); or — the common case — **accept and sharpen.** Most tickets are not clear, and you now hold the hottest context anyone will have on this one. Write what the exploration established back into the body in one edit, so the next reader and the PR's reviewer inherit it instead of re-deriving it. One edit, not a re-authoring.
