...
1. **Fetch Remote Truth:** Before validating a ticket premise, you MUST ensure you are reading the latest truth. You MUST use the `mcp_neo-mjs-github-workflow_get_conversation` tool to fetch the live issue body and comment thread directly from GitHub.
   ...
   - **Readiness Pre-Check:** a ticket carrying an open `blocked_by` edge is not claimable yet. Read what blocks it rather than claiming around it; if the blocker is already resolved, the edge is the thing to correct. **Readiness is a relationship, never a label** — a blocked-by edge clears itself when the blocker closes, which is precisely what a manually-applied state does not do.
...
**Close Policy:**
- **Architecture Exploration / Epic Tickets:** **DO NOT close the ticket.** It must be preserved so the Swarm can formally evaluate the paradox. Record what it waits on via `update_issue_relationship(blocked_by)` and say why in a comment. Do **not** reach for a label: `needs-re-triage` belongs to a different audience (externally-authored tickets below the quality or intent bar — see `ticket-triage`), and applying it to our own architecture work both mislabels the ticket and pollutes that queue.
...
1.  **Blocker Declaration:** Use `update_issue_relationship(relationship_type: 'blocked_by')` to record what the ticket waits on. A ticket nothing blocks is claimable by definition — if you cannot name a blocker, the honest outcome is the critique below, not a suppressed ticket.
