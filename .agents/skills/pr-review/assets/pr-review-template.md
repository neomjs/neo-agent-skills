...
*   **Inputs Read Before Patch:** [Ticket / issue — for a behavior change, its `Design authority:` line and the record it cites — changed-file list, current `dev` source, sibling precedent, source-of-authority substrate read before treating the patch as evidence.]
...
Reference: the Brain-owned [Substrate Evidence Ladder](https://github.com/neomjs/neo-agent-brain/blob/dev/learn/agentos/process/evidence-ladder.md) for L1-L4 ladder + sandbox-vs-achievable ceiling distinction.
